...
assert.equal(
  commands.find((item) => item.command === 'gitMgr.previousFileDiff')?.icon,
  '$(arrow-left)'
);
assert.equal(
  commands.find((item) => item.command === 'gitMgr.nextFileDiff')?.icon,
  '$(arrow-right)'
);
assert.deepEqual(
  editorTitle.map((item) => item.command),
  ['gitMgr.previousFileDiff', 'gitMgr.nextFileDiff']
);
assert.deepEqual(
