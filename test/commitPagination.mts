...
assert.ok(pagedLogArgs.includes('--topo-order'));
assert.equal(pagedLogArgs.includes('--date-order'), false);
assert.deepEqual(
...
const allBranchesLogArgs = buildLogArgs(50, 0, { branch: '__all__' });
assert.ok(allBranchesLogArgs.includes('--branches'));
assert.ok(allBranchesLogArgs.includes('--remotes'));
assert.ok(allBranchesLogArgs.includes('--tags'));
assert.equal(allBranchesLogArgs.includes('--all'), false);
