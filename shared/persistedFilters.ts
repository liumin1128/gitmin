...
export function commitFiltersStateKey(rootPath: string): string {
  return `gitMgr.commitFilters:${rootPath}`;
}
...
export function parsePersistedCommitFilters(value: unknown): CommitFilters {
  ...
  if (typeof value.searchRegex === 'boolean') filters.searchRegex = value.searchRegex;
  ...
  if (typeof value.dateBefore === 'string') filters.dateBefore = value.dateBefore;
  return filters;
}
